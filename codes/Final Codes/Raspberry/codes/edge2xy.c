...
const int penTip = 1; 
const int maxConnectionRange = 12;
...
unsigned char image2[width][height] = {0};
...
void imageReader(unsigned char image[width][height]) ;
...
//  memset(&image[0][0], 0, width*height * sizeof(short int));
  imageReader(image);
  memcpy(image2, image, width*height * sizeof(unsigned char));  
...
//  outputWriter(output);
        ...
        if (blackCounter(i,j) >= minBlackToBegin){
          ...
          if (lengthOfLine(line)>=minLineLen){
            ...
            copyLine (lines[numLines-1],line,0);   //dest, source, dest index         
             ...
             memcpy(image, image2, width*height * sizeof(unsigned char));
           //  recoverLine();
        }
      }
  }
 ...
 printf("Number of lines after finder: %d\n", countOfLines(lines));
 
 // memcpy(lines_connected,lines,2000*20000*2 * sizeof(short int));
  
        
 ///line connector 
  unsigned char linesWaiting [2000];
  ...
  while (numOfReductions > 0) {
    ...
    lines_connected[i-1-numOfReductions][0][0] = 9999;
    copyLines(lines, lines_connected);
  }
  ///end of connector
  
  printf("Number of lines after connector %d\n", countOfLines(lines));
  ...
  void recoverLine (void){
    ...
    for (i=1;i<=length;i++){
      image[line[i-1][0]-1][line[i-1][1]-1] = 1; 
...
void flip(short int line[20000][2]){
    ...
    int i = 0;
    while (i<length/2){
      swap(&line[i][0],&line[length-i-1][0]);
      swap(&line[i][1],&line[length-i-1][1]);
      i++;
    }
}
...
void imageReader(unsigned char image[width][height]){    
     ...
     FILE *fp = fopen("../images/edge.csv", "r");
     if (!fp) {
...
void outputWriter(short int output[], unsigned int count){
     unsigned int i = 0;
     FILE *fp = fopen("../images/fotoxy.csv", "w");
     if (!fp) {
         printf("Can't open file\n");
      //   return 0;
      }        
