...
# if (len(faces_haarcascade) or len(faces_lbpcascade)) == 0:
	# quit()
...
if biggest_mutual_coordinates[0] != -1:
	...
	if biggest_lbp_coordinates[0] != -1 and biggest_lbp_coordinates[5]*biggest_lbp_coordinates[6] > biggest_haar_coordinates[5]*biggest_haar_coordinates[6]:
		...
		cropped_image = image[(biggest_lbp_coordinates[1]-yAdd):(biggest_lbp_coordinates[3]+yAdd),(biggest_lbp_coordinates[0]-xAdd):(biggest_lbp_coordinates[2]+xAdd)]
		...
		cropped_image = image[(biggest_haar_coordinates[1]-yAdd):(biggest_haar_coordinates[3]+yAdd),(biggest_haar_coordinates[0]-xAdd):(biggest_haar_coordinates[2]+xAdd)]
...
scale_percent = 19 * 1500 / grayscale.shape[1] # percent of original size. automatic dist. calibration
...
cv2.imwrite('../images/gray' + '.jpg', scaled_down)
#apply contrast limited adaptive histogram equalization and then general histogram equalization
#for auto-calibrating in different light conditions

#filtered = cv2.GaussianBlur(scaled_down,(5,5),0)
filtered = cv2.bilateralFilter(scaled_down,8,8,50)
cv2.imwrite('../images/blur' + '.jpg', filtered)

hist = cv2.equalizeHist(filtered)
cv2.imwrite('../images/hist' + '.jpg', hist)
clahe = cv2.createCLAHE(clipLimit=1.6, tileGridSize=(20,20))
clahe = clahe.apply(hist)
cv2.imwrite('../images/clahe' + '.jpg', clahe)

edge = cv2.Canny(clahe,10000,21000,apertureSize=7,L2gradient=True)
