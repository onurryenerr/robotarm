function photoToPositions () %#codegen

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%  PHOTO TO ARRAY BEGIN %%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%IMPORT IMAGE
newData1 = importdata('BMP_NEW.bmp');
% Create new variables in the base workspace from those fields.
cdata = fieldnames(newData1);
for i = 1:length(cdata)
    assignin('base', cdata{i}, newData1.(cdata{i}));
end
%%%%
...
minLineLen = 3; % min length of line to record
...
while (positions(j) ~= 2530)
    ...
    output (i) = 570+((2360-570)/180)*trMatrix(positions(j),positions(j+1)); %alpha calibration
    i=i+1;
    output (i) = 570+((2370-570)/180)*trMatrix(positions(j),offset+positions(j+1)); %betta calibration
    i=i+1;  
    j=j+2;
    end
